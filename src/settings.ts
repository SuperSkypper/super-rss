...
export class RssSettingTab extends PluginSettingTab {
    plugin: RssPlugin;
    private activeTab: 'general' | 'template' | 'feeds' = 'general';
    ...
    display(): void {
        ...
        const createTabBtn = (id: 'general' | 'template' | 'feeds', label: string) => {
        ...
        const refresh = this.display.bind(this);

        switch (this.activeTab) {
            case 'general':
                renderGeneralTab(containerEl, this.app, this.plugin, cardStyle);
                break;
            case 'template':
                renderGlobalTemplateTab(containerEl, this.plugin, cardStyle, autoResize);
                break;
            case 'feeds':
                renderMyFeedsTab(containerEl, this.app, this.plugin, cardStyle, refresh);
                break;
        }
    }
}