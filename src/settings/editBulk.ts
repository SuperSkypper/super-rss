...
type TemplateValueKey = 'titleTemplate' | 'frontmatterTemplate' | 'contentTemplate';
...
interface BulkEditValues {
    ...
    updateIntervalUnit: IntervalUnit;
    autoCleanupValue: number;
    autoCleanupUnit: IntervalUnit;
    autoCleanupDateField: CleanupDateField;
    tagShorts: boolean | undefined;
    skipShorts: boolean | undefined;
...
export function openBulkEditModal(
    ...
    class BulkEditModal extends Modal {
        ...
        onOpen() {
            ...
            applyCssText(hint, 'font-size: 0.83em; color: var(--text-muted); margin: -4px 0 12px; flex-shrink: 0;');
        ...
        private wrapWithApply(
            ...
            applyCssText(row, 'display: flex; align-items: flex-start; gap: 10px; margin-bottom: 12px;');

            const cbWrap = row.createDiv();
            applyCssText(cbWrap, 'display: flex; align-items: center; padding-top: 14px; flex-shrink: 0;');
            const cb = cbWrap.createEl('input', { type: 'checkbox' });
            applyCssText(cb, 'cursor: pointer; width: 15px; height: 15px; margin: 0;');
        ...
        private cardHeader(card: HTMLElement, icon: string, title: string, desc?: string): void {
            ...
            if (desc) {
                ...
                applyCssText(descEl, 'color: var(--text-muted); font-size: 0.82em; margin: 0 0 8px;');
            }
        }
        ...
        private renderFeedTab(container: HTMLElement) {
            ...
            this.wrapWithApply(container, 'groupId', (card, markDirty) => {
                ...
                applyCssText(select, `width: 100%; box-sizing: border-box; font-size: ${inputFontSize()};`);
            ...
            this.wrapWithApply(container, 'folder', (card, markDirty) => {
                ...
                applyCssText(input, `width: 100%; box-sizing: border-box; font-size: ${inputFontSize()};`);
        ...
        private renderOverridesTab(container: HTMLElement) {
            ...
            const sectionHeader = (text: string) => {
                ...
                applyCssText(h, 'margin: 16px 0 8px; color: var(--text-muted); font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.05em;');
            };
            ...
            this.wrapWithApply(container, 'updateIntervalValue', (card, markDirty) => {
                ...
                const row = card.createDiv();
                applyCssText(row, 'display: flex; align-items: center; gap: 8px;');
                ...
                applyCssText(input, `width: 80px; box-sizing: border-box; font-size: ${inputFontSize()};`);
                ...
                applyCssText(unitSelect, `font-size: ${inputFontSize()};`);
                ...
                unitSelect.addEventListener('change', () => { this.values.updateIntervalUnit = unitSelect.value as IntervalUnit; markDirty(); });
            });
            ...
            this.wrapWithApply(container, 'autoCleanupValue', (card, markDirty) => {
                ...
                applyCssText(deleteRow, 'display: flex; align-items: center; gap: 8px; margin-bottom: 10px;');

                const deleteLabel = deleteRow.createEl('span', { text: 'Delete articles older than' });
                applyCssText(deleteLabel, 'font-size: 0.85em; color: var(--text-muted); white-space: nowrap;');
                ...
                input.inputMode     = 'numeric';
                applyCssText(input, `width: 70px; box-sizing: border-box; font-size: ${inputFontSize()};`);
                input.addEventListener('input', () => {
                    const v = parseInt(input.value);
                ...
                unitSelect.addEventListener('change', () => { this.values.autoCleanupUnit = unitSelect.value as IntervalUnit; markDirty(); });

                const criterionWrapper = card.createDiv();
                applyCssText(criterionWrapper, 'margin-left: 20px; border-left: 3px solid var(--interactive-accent); padding-left: 12px;');
                const criterionLabel = criterionWrapper.createEl('p', { text: 'Date criterion — which date field to use.' });
                applyCssText(criterionLabel, 'font-size: 0.82em; color: var(--text-muted); margin: 0 0 6px;');
                ...
                applyCssText(dateSelect, `font-size: ${inputFontSize()};`);
                ...
                dateSelect.addEventListener('change', () => { this.values.autoCleanupDateField = dateSelect.value as CleanupDateField; markDirty(); });
            });
        ...
        private renderFooter(contentEl: HTMLElement) {
            ...
            applyCssText(footer, 'margin-top: 12px; flex-shrink: 0; display: flex; justify-content: flex-end; gap: 8px;');
